...
public class ResultHandler {
    ...
    private PageResult getNoAggResult(JSONObject result) {
        ...
        Integer count = result.getJSONObject("hits").getInteger("total");
        JSONArray hits = result.getJSONObject("hits").getJSONArray("hits");
        for(int i=0;i<hits.size();i++){
            JSONObject jsonObject = (JSONObject)hits.get(i);
            jsonObjects.add(jsonObject.getJSONObject("_source"));
        }
    ...
    private PageResult getAggResult(JSONObject result) {
        ...
        Integer count = jsonObjects.size();
        if(model!=null){
            PageResult pageResult = new PageResult();
    ...
    private List<JSONObject> analyzeAnalyze0(JSONObject result){
        ...
        if(conditions!=null){
            ...
            for(Condition condition : conditions){
                ...
                jsonObject.put(value+"_"+field,aggregations.getJSONObject(value+"_"+field).getString("value"));
            }
        ...
        for(int i=0;i<buckets.size();i++){
            ...
            if(!CollectionUtils.isEmpty(conditions)){
                for(Condition condition : conditions){
                    String field  = condition.getField();
                    String value = condition.getValue();
                    jsonObject.put(value+"_"+field,bucket.getJSONObject(value+"_"+field).getString("value"));
                }
            }
            jsonObject.put("group_"+groups.get(0)+"_value",bucket.getString("key"));
            jsonObject.put("group_"+groups.get(0)+"_count",bucket.getString("doc_count"));
            jsonObjects.add(jsonObject);
        }
        return jsonObjects;
    }
    ...
    private List<JSONObject> analyzeAnalyze2(JSONObject result){
        ...
        for(int i=0;i<buckets1.size();i++){
            ...
            for(int j=0;j<buckets2.size();j++){
                ...
                jsonObject2.put("group_"+groups.get(1)+"_value",bucket2.getString("key"));
                jsonObject2.put("group_"+groups.get(1)+"_count",bucket2.getString("doc_count"));
                if(!CollectionUtils.isEmpty(conditions)){
                    for(Condition condition : conditions){
                        String field  = condition.getField();
                        String value = condition.getValue();
                        jsonObject2.put(value+"_"+field,bucket2.getJSONObject(value+"_"+field).getString("value"));
                    }
                }
                jsonObject2.put("group_"+groups.get(0)+"_value",key1);
                jsonObject2.put("group_"+groups.get(0)+"_count",value1);
                jsonObject2.put("group_"+groups.get(1)+"_value",bucket2.getString("key"));
                jsonObject2.put("group_"+groups.get(1)+"_count",bucket2.getString("doc_count"));
                ...
                for(int x=0;x<buckets3.size();x++){
                    ...
                    if(!CollectionUtils.isEmpty(conditions)){
                        for(Condition condition : conditions){
                            String field  = condition.getField();
                            String value = condition.getValue();
                            jsonObject3.put(value+"_"+field,bucket3.getJSONObject(value+"_"+field).getString("value"));
                        }
                     }
                    jsonObject3.put("group_"+groups.get(0)+"_value",key1);
                    jsonObject3.put("group_"+groups.get(0)+"_count",value1);
                    jsonObject3.put("group_"+groups.get(1)+"_value",key2);
                    jsonObject3.put("group_"+groups.get(1)+"_count",value2);
                    jsonObject3.put("group_"+groups.get(2)+"_value",bucket3.getString("key"));
                    jsonObject3.put("group_"+groups.get(2)+"_count",bucket3.getString("doc_count"));
